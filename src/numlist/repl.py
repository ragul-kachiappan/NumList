...
def print_history(manager: NumberListManager) -> None:
    ...
    print_formatted_text(HTML('<ansigreen><b>📅 Recent Numbers (last 10):</b></ansigreen>'))
...
def handle_remove_command(manager: NumberListManager, user_input: str) -> None:
    ...
    try:
        number = int(parts[1])
        if number <= 0:
            print_formatted_text(HTML('<ansired>❌ Error: Only positive integers are allowed.</ansired>'))
            return
            
        if manager.remove_number(number):
            print_formatted_text(HTML(f'<ansigreen>✅ Successfully removed {number} from the list!</ansigreen>'))
        else:
            print_formatted_text(HTML(f'<ansired>❌ Error: {number} is not in the list!</ansired>'))
    except ValueError:
        print_formatted_text(HTML(f'<ansired>❌ Invalid number: "{parts[1]}"</ansired>'))
...
def handle_clear_command(manager: NumberListManager) -> None:
    ...
    confirm = prompt(HTML('<ansired><b>⚠️  Clear all numbers? (yes/no): </b></ansired>')).lower()
    if confirm in ['yes', 'y']:
...
def validate_and_add_number(manager: NumberListManager, user_input: str) -> None:
    ...
    if '.' in user_input:
        print_formatted_text(HTML(f'<ansired>❌ Error: "{user_input}" is not a valid positive integer. Decimals are not allowed.</ansired>'))
        return
    ...
    try:
        ...
        if number <= 0:
            print_formatted_text(HTML(f'<ansired>❌ Error: {number} is not positive. Only positive integers (> 0) are allowed.</ansired>'))
            return
        
        if manager.add_number(number):
            print_formatted_text(HTML(f'<ansigreen>✅ Successfully added {number} to the list!</ansigreen>'))
        else:
            print_formatted_text(HTML(f'<ansired>❌ Error: {number} is already in the list!</ansired>'))
    
    except ValueError:
        print_formatted_text(HTML(f'<ansired>❌ Invalid input: "{user_input}" is not a valid positive integer.</ansired>'))
